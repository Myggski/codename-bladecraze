...
local function _update_sprite_batch(button)
  ...
  nine_slice_scaling.set_sprite_batch(
    button, texture:getWidth() / table.get_size(BUTTON_ANIMATION_STATE_TYPES),
...
local function _add_text(button, text_list)
  ...
  local animation_y_offset = button.animation_state - 1 -- When the button is being pressed down, the text should follow
  local text_x, text_y = button_center_x - text_width / 2, (button_center_y - text_height / 2) + animation_y_offset

  return text_list:add(button.text, math.round(text_x), math.floor(text_y))
end
