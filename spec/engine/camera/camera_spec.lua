...
insulate("camera", function()
  ...
  describe("look_at", function()
    ...
    describe("is_outside_camera_view", function()
      describe("when the rectangle top-left is in the center of the screen", function()
        it("should not be outside of camera view", function()
          local rect = rectangle(0, 0, 1, 1)

          assert.is.falsy(camera:is_outside_camera_view(rect))
        end)
      end)

      describe("when the rectangles top-left is in the bottom-right corner of the screen", function()
        it("should be outside of camera view", function()
          local world_x, world_y = world_grid:world_to_grid(camera:get_screen_game_half_size())
          local rect = rectangle(world_x, world_y, 1, 1)

          assert.is.truthy(camera:is_outside_camera_view(rect))
        end)
      end)

      describe("when the rectangles top-left is in the top-left corner of the screen", function()
        it("should not be outside of camera view", function()
          local world_x, world_y = world_grid:world_to_grid(camera:get_screen_game_half_size())
          local rect = rectangle(-world_x, -world_y, 1, 1)
